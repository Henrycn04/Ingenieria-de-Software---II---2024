<template>
    ...
    <div class="row justify-content-end">
        <div class="col-2">
            <a href="/pais">
                <button type="button" class="btn btn-outline-secondary float-right"> Agregar pais </button>
            </a>
        </div>
    </div>
        <table
        ...
        >
            ...
            <tbody id="list">
                <tr v-for="(pais, index) of paises" :key="index" id="">
                    <td>{{ pais.nombre }}</td>
...
import axios from 'axios';
    export default {
        ...
        methods: {
            /* splice sirve para eliminar un elemento por indice de cierto array en el data UTIL!!! */
            eliminar: function (indice) {
                this.paises.splice(indice, 1); 
            },
            obtenerTareas() {
                axios.get("https://localhost:7206/api/Paises").then((response) => {
